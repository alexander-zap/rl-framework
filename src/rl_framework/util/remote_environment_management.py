...
def start_as_remote_environment(local_environment: Environment, url: Text) -> Tuple[grpc.Server, int]:
    """
    ...
    Returns:
        ...
        port: Port (on the remote machine URL) for communication with the remote environment.
    ...
    port = server.add_secure_port(f"{url}:0", grpc.local_server_credentials())
    server.start()
