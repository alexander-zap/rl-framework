...
class EpisodeSequence(Iterable[GenericEpisode], Sized):
    ...
    @staticmethod
    def from_dataset(file_path) -> "EpisodeSequence":
        """
        Load recorded episode sequences based on provided huggingface dataset path.
        ...
        def generate_episodes() -> Generator[GenericEpisode, None, None]:
            for trajectory in trajectories:
                ...
                episode_sequence: GenericEpisode = list(
                    zip(*[obs, acts, next_obs, rews, terminations, truncations, infos])
                )
                yield episode_sequence

        trajectories: Sequence[imitation.data.types.TrajectoryWithRew] = serialize.load_with_rewards(file_path)
        episode_sequence = EpisodeSequence()
        episode_sequence._episode_generator = generate_episodes()
